...
public class BrowseFacetOptionsResponseTest {
    ...
    public void createBrowseFacetOptionsResponseShouldReturnAResult() throws Exception {
        ...
        BrowseFacetOptionsResponse response =
                ConstructorIO.createBrowseFacetOptionsResponse(string);
        ...
        assertEquals(
                "type exists", "multiple", response.getResponse().getFacets().get(0).getType());
        assertEquals(
                "4 options exists",
                4,
                response.getResponse().getFacets().get(0).getOptions().size());
    }
}
