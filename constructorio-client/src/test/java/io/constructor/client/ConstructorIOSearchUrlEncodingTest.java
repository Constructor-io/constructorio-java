...
public class ConstructorIOSearchUrlEncodingTest {
    ...
    public void SearchWithPlusShouldBeEncodedInUrl() throws Exception {
        ...
        String expectedPath = String.format("/search/r%%2Bco?key=%s&c=ciojava-5.17.0&section=Products&page=1&num_results_per_page=30", apiKey);
    ...
    public void SearchWithSpaceShouldBeEncodedInUrl() throws Exception {
        ...
        RecordedRequest recordedRequest = mockServer.takeRequest();
        String expectedPath = String.format("/search/r%%20co?key=%s&c=ciojava-5.17.0&section=Products&page=1&num_results_per_page=30", apiKey);
        String actualPath = recordedRequest.getPath();
        assertEquals("recorded request is encoded correctly", actualPath, expectedPath);
    }
    ...
    public void SearchWithSlashShouldBeEncodedInUrl() throws Exception {
        ...
        RecordedRequest recordedRequest = mockServer.takeRequest();
        String expectedPath = String.format("/search/r%%2Fco?key=%s&c=ciojava-5.17.0&section=Products&page=1&num_results_per_page=30", apiKey);
        String actualPath = recordedRequest.getPath();
        assertEquals("recorded request is encoded correctly", actualPath, expectedPath);
    }
    ...
    public void SearchWithSingleQuoteShouldBeAllowedInUrl() throws Exception {
        ...
        RecordedRequest recordedRequest = mockServer.takeRequest();
        String expectedPath = String.format("/search/r'co?key=%s&c=ciojava-5.17.0&section=Products&page=1&num_results_per_page=30", apiKey);
        String actualPath = recordedRequest.getPath();
        assertEquals("recorded request is encoded correctly", actualPath, expectedPath);
    }
}