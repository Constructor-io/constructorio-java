...
public class ConstructorIO {
   ...
   *     HTTPS.
   * @param host The host of the autocomplete service that you are using. It is recommended that you
   *     let this value be null, in which case the host defaults to the Constructor.io autocomplete
   *     service at ac.cnstrc.com.
   * @param constructorToken The token provided by Constructor to identify your company's traffic if
   *     proxying requests for results
   */
   ...
   * @param force whether or not the system should process the request even if it will invalidate a
   *     large number of existing variations.
   * @param notificationEmail An email address where you'd like to receive an email notification in
   *     case the task fails.
   * @return true if working
  ...
  public boolean createOrReplaceItems(
    ...
    try {
      ...
      if (notificationEmail != null) {
        url = url.newBuilder().addQueryParameter("notification_email", notificationEmail).build();
      }
  ...
  public boolean deleteVariations(
      ConstructorVariation[] variations, String section, Boolean force, String notificationEmail)
      throws ConstructorException {
    try {
  ...
  public boolean deleteVariations(ConstructorVariation[] variations, String section, Boolean force)
      throws ConstructorException {
  ...
  public boolean updateVariations(ConstructorVariation[] variations, String section, Boolean force)
      throws ConstructorException {
   ...
   *
   * <p>Note that if you're making an autocomplete request for a website, you should definitely use
   * our javascript client instead of doing it server-side! That's important. That will be a solid
   * latency difference.
   *
  ...
  public String autocompleteAsJSON(AutocompleteRequest req, UserInfo userInfo)
    ...
    try {
      ...
      for (String hiddenField : req.getHiddenFields()) {
        url = url.newBuilder().addQueryParameter("fmt_options[hidden_fields]", hiddenField).build();
      }

      for (String filterName : req.getFilters().keySet()) {
        for (String facetValue : req.getFilters().get(filterName)) {
          url =
              url.newBuilder().addQueryParameter("filters[" + filterName + "]", facetValue).build();
        }
      }
  ...
  protected Request createSearchRequest(SearchRequest req, UserInfo userInfo)
    ...
    try {
      ...
      if (req.getGroupId() != null) {
        url = url.newBuilder().addQueryParameter("filters[group_id]", req.getGroupId()).build();
      }

      for (String facetName : req.getFacets().keySet()) {
        for (String facetValue : req.getFacets().get(facetName)) {
          url =
              url.newBuilder().addQueryParameter("filters[" + facetName + "]", facetValue).build();
        }
      }
      ...
      for (String hiddenFacet : req.getHiddenFacets()) {
        url = url.newBuilder().addQueryParameter("fmt_options[hidden_facets]", hiddenFacet).build();
      }
      ...
      if (StringUtils.isNotBlank(req.getSortBy())) {
        url =
            url.newBuilder()
                ...
                .addQueryParameter(
                    "sort_order", req.getSortAscending() ? "ascending" : "descending")
                .build();
      }

      if (req.getCollectionId() != null) {
        url = url.newBuilder().addQueryParameter("collection_id", req.getCollectionId()).build();
      }
      ...
      if (req.getPage() != 1) {
        url = url.newBuilder().addQueryParameter("page", String.valueOf(req.getPage())).build();
      }

      if (req.getOffset() != 0) {
        url = url.newBuilder().addQueryParameter("offset", String.valueOf(req.getOffset())).build();
      }
  ...
  /**
   * Transforms a JSON string to a new JSON string for easy Gson parsing into a Task response. Using
   * JSON objects to achieve this is considerably less error prone than attempting to do it in a
   * Gson Type Adapter.
   */
        ...
        if (!propertyName.matches("(description|id|url|image_url|group_ids|facets|keywords)")) {
          ...
          String camelCasePropertyName =
              propertyName.replaceFirst(
                  "_[a-z]",
                  String.valueOf(
                      Character.toUpperCase(propertyName.charAt(propertyName.indexOf("_") + 1))));
  ...
  public String replaceCatalog(CatalogRequest req) throws ConstructorException {
    try {
      ...
      HttpUrl.Builder urlBuilder = url.newBuilder().addQueryParameter("section", req.getSection());
  ...
  public String updateCatalog(CatalogRequest req) throws ConstructorException {
    try {
      ...
      RequestBody requestBody = multipartBuilder.build();
      Request request = this.makeAuthorizedRequestBuilder().url(url).patch(requestBody).build();
