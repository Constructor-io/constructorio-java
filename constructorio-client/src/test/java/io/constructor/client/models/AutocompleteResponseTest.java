...
public class AutocompleteResponseTest {
    ...
    public void newFromGsonShouldReturnResponse() throws Exception {
        ...
        assertEquals("search suggestions exist", response.getSections().get("Search Suggestions").length, 8);
        assertEquals("product suggestions exist", response.getSections().get("Products").length, 6);
