...
public class BrowseResponseTest {
  ...
  public void createBrowseResponseShouldReturnAResult() throws Exception {
    ...
    assertEquals(
        "total number of results", (int) response.getResponse().getTotalNumberOfResults(), 562);
    assertTrue(
    ...
    assertEquals(
        "request filter value exists", response.getRequest().get("browse_filter_value"), "Blue");
  }
  ...
  public void createBrowseResponseShouldReturnAResultWithSortOptions() throws Exception {
    ...
    BrowseResponse response = ConstructorIO.createBrowseResponse(string);
    assertEquals(
        "browse result [sort options] exists", response.getResponse().getSortOptions().size(), 4);
    assertEquals(
