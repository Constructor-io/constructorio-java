...
public class ConstructorIOBasicTest {
  ...
  public void makeUrlShouldReturnAUrl() throws Exception {
    ...
    assertEquals("version is set", url.queryParameter("c"), "ciojava-5.22.0");
  ...
  public void getVersionShouldReturnClientVersion() throws Exception {
    ConstructorIO constructor = new ConstructorIO(token, apiKey, true, null);
    assertEquals("grabs version from pom.xml", constructor.getVersion(), "ciojava-5.22.0");
  }
  ...
  public void getResponseBodyShouldReturnExceptionOn404() throws Exception {
    thrown.expect(ConstructorException.class);
    thrown.expectMessage(
        "[HTTP 404] You're trying to access an invalid endpoint. Please check documentation for"
            + " allowed endpoints.");
  ...
  public void getResponseBodyShouldReturnExceptionWithErrorCodeOn401() throws Exception {
    try {
      ...
      assertEquals(
          "[HTTP 401] Invalid auth_token. If you've forgotten your token, you can generate a new"
              + " one at constructor.io/dashboard",
          e.getMessage());
    }
  }
